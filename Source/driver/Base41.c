...
{
	uint16 ByteConnect=0,index=0;
	uint8 First=0,Second=0,Third=0;


	for(int i=0;i<n;i+=2)
	{
		ByteConnect = input[i] <<8 | input[i+1];
		Third = ByteConnect / 1681;
		ByteConnect %= 1681;
		Second = ByteConnect / 41;
		First = ByteConnect % 41;

		output[index++] = Base41Table[First];
		output[index++] = Base41Table[Second];
		output[index++] = Base41Table[Third];
	}
	...
	{
	    output[index++] = Base41Table[input[n] % 41];
	    output[index++] = Base41Table[input[n] / 41];
	}
}
