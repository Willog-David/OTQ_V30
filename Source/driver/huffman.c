...
uint8 _Huffman_MakeTree( huff_sym_t *sym, huff_bitstream_t *stream )
{
  huff_encodenode_t nodes[MAX_TREE_NODES], *node_1, *node_2, *root;
  unsigned int k=0, num_symbols=0, nodes_left=0, next_idx=0;
  ...
  else
  {
    ...
    //NRF_LOG_INFO("single node create");
  }
  return 0;
}
  ...
  huff_bitstream_t *stream, unsigned int *nodenum )
{
  ...
  *nodenum = *nodenum + 1;
  ...
  if(*nodenum >= MAX_TREE_NODES)return 0;			// 노드가 max를 넘어갈경우 fault 발생
  ...
  unsigned int insize )
{
  ...
  /* Build Huffman tree */
  ret = _Huffman_MakeTree( sym, &stream );
  if(ret == 0xff)
  {
	  return 0xffff;
  }
  ...
  for(k = 0; k<256; k++)
  {
	  MaxLength = MaxLength + sym[k].Count * sym[k].Bits;
  }
  if(MaxLength > 600 * 8)			// huffman buffer max size 550
  {
	  return 0xffff;
  }
  /* Encode input stream */
