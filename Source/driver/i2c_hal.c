...
// SDA on port B, bit 9
#define SDA_LOW()  0//(GPIOA->BSRR = 0x04000000) // set SDA to low
#define SDA_OPEN() 0//(GPIOA->BSRR = 0x00000400) // set SDA to open-drain
#define SDA_READ   0//(GPIOA->IDR  & 0x0400)     // read SDA

// SCL on port B, bit 8                /* -- adapt the defines for your uC -- */
#define SCL_LOW()  0//(GPIOA->BSRR = 0x02000000) // set SCL to low
#define SCL_OPEN() 0//(GPIOA->BSRR = 0x00000200) // set SCL to open-drain
#define SCL_READ   0//(GPIOA->IDR  & 0x0200)     // read SCL

//------------------------------------------------------------------------------
void I2c_Init(void)                       /* -- adapt the init for your uC -- */
{
	/*
  RCC->APBENR |= 0x00000008;  // I/O port B clock enabled
