...
ret_code_t twi_write(uint8 TWIAddr,uint16 RegAddr,uint8 RegAddrLen, uint8 *data, uint16 len)
{
    uint8_t *reg = malloc(len+2);
    uint8_t AddrLen=0;
    ...
    twi_err_code = nrf_drv_twi_tx(&m_twi, TWIAddr, reg, len + AddrLen, false);
...
ret_code_t twi_read(uint8 TWIaddr,uint16 RegAddr,uint8 RegAddrLen, uint8 *data, uint16 len)
{
    m_xfer_done = false;
    uint8 RegAddr_8b;
    uint16 RegAddr_16b;
    ...
    twi_err_code = nrf_drv_twi_tx(&m_twi, TWIaddr, (uint8_t*)&RegAddr_16b, 2, false);
    }
    else
    {
    RegAddr_8b = RegAddr;
    twi_err_code = nrf_drv_twi_tx(&m_twi, TWIaddr, &RegAddr_8b, 1, false);
    }
    ...
    m_xfer_done = false;
    twi_err_code = nrf_drv_twi_rx(&m_twi, TWIaddr, data, len);
    do{
        nrf_delay_ms(1);
...
ble_srv_error_handler_t nrf_qwr_error_handler(uint32_t error);
